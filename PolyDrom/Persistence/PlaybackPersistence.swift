...
extension AppCoordinator {
    ...
    func reconcilePersistedPlaybackQueue(for generation: UInt) async {
        guard let state = pendingPlaybackRestore,
              let client,
        ...
        do {
            var availableEntries: [PlaybackQueueEntry] = []
            var removedSongTitles: [String] = []

            for entry in state.queue {
                guard isCurrentSession(generation, serverKey: activeServerKey) else { return }
                if let song = try await client.songMetadata(for: entry.song.id) {
                    availableEntries.append(PlaybackQueueEntry(id: entry.id, song: song))
                } else {
                    removedSongTitles.append(entry.song.title)
                }
            }

            guard isCurrentSession(generation, serverKey: activeServerKey) else { return }

            let currentEntry = state.currentQueueEntryID.flatMap { entryID in
                availableEntries.first { $0.id == entryID }
            } ?? state.currentSong.flatMap { song in
                availableEntries.first { $0.song.id == song.id }
            }
            let currentSong = currentEntry?.song
            let currentQueueEntryID = currentEntry?.id
            let position = currentSong?.id == state.currentSong?.id ? state.position : 0
            let normalizedState = PersistedPlaybackState(
                serverKey: activeServerKey,
                queue: availableEntries,
                currentQueueEntryID: currentQueueEntryID,
                currentSong: currentSong,
            ...
            playbackQueue = availableEntries
            self.currentPlaybackQueueEntryID = currentQueueEntryID
            ...
            if !removedSongTitles.isEmpty {
                let count = removedSongTitles.count
                statusMessage = count == 1
                    ? "Removed unavailable song from queue."
                    : "Removed \(count) unavailable songs from queue."
            }
        } catch {
            AppLog.playback.error(
                "Could not reconcile persisted playback queue: \(error.localizedDescription, privacy: .private)"
            )
        }
    }
